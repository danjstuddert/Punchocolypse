...
public class InventorySlot : MonoBehaviour
{
    ...
    private void Start()
    {
        if (startingObject)
        {
            ...
            if (hideOnAttach)
            {
                attachedObject.GetComponent<MeshRenderer>().enabled = false;
            ...
            if (attachedObject.GetComponent<Attachable>())
            {
                if (hideOnAttach)
                    attachedObject.GetComponent<Attachable>().OnAttachedToFace();
                else
                    attachedObject.GetComponent<Attachable>().OnAttached();
            }
        }
    ...
    public void DetachHeldObject()
    {
        if (hideOnAttach)
        {
            attachedObject.GetComponent<MeshRenderer>().enabled = true;
        ...
        {
            if (hideOnAttach)
                attachedObject.GetComponent<Attachable>().OnDetachedFromFace();
            else
                attachedObject.GetComponent<Attachable>().OnDetached();
        }
