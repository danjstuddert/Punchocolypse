...
public class Eatable : MonoBehaviour {
    ...
    [Header("Overlays")]
    [SerializeField] private GameObject eatableOverlay;
    ...
    private void Eat(Affectable affectable)
    {
        ...
        if (eatableOverlay)
        {
            GameObject overlay = SimplePool.Spawn(eatableOverlay, GameObject.FindWithTag("PlayerHead").transform.position, GameObject.FindWithTag("PlayerHead").transform.rotation);
            overlay.transform.parent = GameObject.FindWithTag("PlayerHead").transform;

            overlay.GetComponent<Overlay>().Init(overlayFadeTime, overlayDisplayTime);
        }
    ...
    private IEnumerator StartEat(Transform eatingCollider, Affectable other)
    {
        ...
        if (eatenParticle)
        {
            GameObject p = ParticleController.Instance.SpawnParticle(eatenParticle, transform.position, Quaternion.identity);
