...
namespace Valve.VR.InteractionSystem
{
    public class Arrow : Hoverable
    {
        ...
        [SerializeField] private float minImpactSoundVelocity;
        ...
        protected virtual void OnCollisionEnter(Collision collision)
        {
            if (inFlight)
            {
               ...
               // bool canImpale = (impalePhysMaterial != null && collision.collider.sharedMaterial == impalePhysMaterial && rbSpeed > minImpaleVelocity);
                ...
                if (travelledFrames < 1 && !canStick)// && !canImpale)
                {
                ...
                else
                {
                    ...
                    Vector3 hitpoint = collision.contacts[0].point - transform.forward * (0.75f - (Util.RemapNumberClamped(prevVelocity.magnitude, 0f, 10f, 0.0f, 0.1f) + Random.Range(0.0f, 0.05f)));
                    if (arrowHeadRB.velocity.sqrMagnitude >= minImpactSoundVelocity * minImpactSoundVelocity)
                        ImpactController.Instance.PlayImpact(collision.collider.sharedMaterial,/* arrowHeadRB.transform.position*/ hitpoint, arrowHeadRB.velocity);
                }
                ...
                if (arrowHeadRB.velocity.sqrMagnitude >= minDamageVelocity)
                {
                    ...
                    for (int i = 0; i < affectables.Length; i++)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                affectables[i].Damage(damage, arrowHeadRB.transform.position, arrowHeadRB.velocity);
                ...
                if (canStick)
                {
                    ...
                    if(collision.gameObject.GetComponent<AnchorPoint>())
                        Anchor(collision);
                }
        ...
        protected virtual void Anchor(Collision collision)
        {
            ...
            {
                GameObject anchor = GameObject.FindWithTag("BowAnchor");

                if (anchor == null)
                    return;

                anchor.GetComponent<BowAnchor>().BreakAnchor();
            }

            collision.gameObject.GetComponent<AnchorPoint>().Anchored(transform.position);
        }
