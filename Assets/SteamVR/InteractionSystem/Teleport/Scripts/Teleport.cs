...
namespace Valve.VR.InteractionSystem
{
    ...
    {
        [SerializeField] private bool forceDisabled;
        ...
        [Header("Oculus Exension")]
        [Range(0.1f, 1f), SerializeField]
        private float yTeleportSensitivity;
        ...
        public bool Disabled { get { return forceDisabled; } }
        public bool TeleportButtonHeld { get { return visible; } }
        ...
        private bool displayingPointer;
        ...
        private void Start()
        {
            if(GameObject.FindWithTag("Level") != null)
                teleportMarkers = GameObject.FindWithTag("Level").GetComponentsInChildren<TeleportMarkerBase>();
        ...
        public void PresendPlayer(TeleportMarkerBase pointedAt)
        {
            PlayerPre.Send(pointedAt);

            SteamVR_Fade.Start(Color.clear, currentFadeTime);
        }

        public void SendPlayer(TeleportMarkerBase pointedAt)
        {
            Player.Send(pointedAt);
        }
        ...
        private void Update()
        {
            ...
            foreach (Hand hand in player.hands)
            {
                ...
                if (visible)
                {
                    if (vRDevice == VRDevice.Vive && WasTeleportButtonReleased(hand) ||
                        vRDevice == VRDevice.Occulus && hand.controller.GetAxis().y < yTeleportSensitivity && displayingPointer)
                    {
                        if (pointerHand == hand) //This is the pointer hand
                        {
                            displayingPointer = false;
                            TryTeleportPlayer();
                        }
                    }
                }

                if (vRDevice == VRDevice.Vive && (WasTeleportButtonPressed(hand) && (player.Rotation.CanRotate == false || hand.controller.GetAxis().x < player.Rotation.RotationSensitivity)|| IsTeleportButtonDown(hand)) ||
                    (vRDevice == VRDevice.Occulus && hand.controller.GetAxis().y >= yTeleportSensitivity))
                {
                    displayingPointer = true;
                    newPointerHand = hand;
                }
            }
        ...
        private void UpdatePointer()
        {
            ...
            if ((dotForward > 0 && dotUp > 0.8f) || (dotForward < 0.0f && dotUp > 0.75f))
            {
            ...
            if (teleportArc.DrawArc(out hitInfo))
            {
                ...
                Collider[] hitColliders = Physics.OverlapSphere(hitInfo.point, wallCheckRadius, traceLayerMask);

                foreach (Collider col in hitColliders)
                {
                    if (col.GetComponent<TeleportMarkerBase>())
                        continue;

                    // If not above the hit point we don't care about it
                    if (col.bounds.center.y <= hitInfo.point.y)
                        continue;
                ...
                {
                    hitTeleportMarker = hitInfo.collider.GetComponent<TeleportMarkerBase>();
                    if (hitTeleportMarker == null)
                        hitTeleportMarker = hitInfo.collider.GetComponentInChildren<TeleportMarkerBase>();
                }
        ...
        private void HidePointer()
        {
            ...
            if(teleportMarkers != null && teleportMarkers.Length > 0)
            {
        ...
        private void PlayPointerHaptic(bool validLocation)
        {
            ...
            {
                if (validLocation)
                    pointerHand.controller.TriggerHapticPulse(1000);
                else
                    pointerHand.controller.TriggerHapticPulse(200);
            }
        }
        ...
        private void InitiateTeleportFade()
        {
            ...
            this.Invoke(TeleportPlayer, currentFadeTime);
        }
        ...
        private void HighlightSelected(TeleportMarkerBase hitTeleportMarker)
        {
            ...
            {
                if ((pointedAtPosition - prevPointedAtPosition).sqrMagnitude > 1.0f * 1.0f)
                {
                    prevPointedAtPosition = pointedAtPosition;
                    PlayPointerHaptic(!hitTeleportMarker.Locked);
                }
