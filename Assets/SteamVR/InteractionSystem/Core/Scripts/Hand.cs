...
namespace Valve.VR.InteractionSystem
{
    public class Hand : MonoBehaviour
    {
        ...
        public Transform hoverRayLocation;
                ...
                if (_hoveringInteractable != value)
                {
                    if (_hoveringInteractable != null)
                    {
                        ...
                        if (hoverables.Length <= 0)
                            hoverables = _hoveringInteractable.gameObject.GetComponentsInChildren<Hoverable>();
        ...
        public void AttachObject(GameObject objectToAttach, AttachmentFlags flags = defaultAttachmentFlags, string attachmentPoint = "")
        {
            ...
            if (currentAttachedObject)
            {
                ...
                if (hoverables.Length <= 0)
                    hoverables = currentAttachedObject.GetComponentsInChildren<Hoverable>();
            ...
            if (objectToAttach.GetComponent<Throwable>())
            {
                ...
                for (int i = 0; i < overlappingColliders.Length; i++)
                {
                    if (overlappingColliders[i] == null || overlappingColliders[i].transform != objectToAttach.transform)
                        continue;
                ...
                // If we didn't find anything it must be at range
                if(foundCollider == false)
                {
                    RaycastHit[] colliders = Physics.SphereCastAll(hoverRayLocation.position, hoverSphereRadius,
                    hoverRayLocation.forward, maxThrowableDistance, hoverLayerMask);

                    for (int i = 0; i < colliders.Length; i++)
                    {
                        if (colliders[i].transform != objectToAttach.transform)
                            continue;

                        grabPoint = colliders[i].transform.position.DirectionTo(colliders[i].point);

                        float distance = grabPoint.magnitude;

                        // Work out where to place empty game object
                        grabPoint = objectToAttach.transform.position + ((grabPoint / distance) * distance);
                        break;
                    }
                ...
                parent.transform.name = "Grabbed Parent";
                parent.transform.position = grabPoint;

                // Parent new object to hand
                parent.transform.SetParent(transform, true);

                // Parent object to attach to new object
                objectToAttach.transform.SetParent(parent.transform);

                // Update attachedObject to new object
            ...
            if (h.Length <= 0)
                h = objectToAttach.GetComponentsInChildren<Hoverable>();
        ...
        public void DetachObject(GameObject objectToDetach, bool restoreOriginalParent = true)
        {
            ...
            int index = objectToDetach.transform.parent && objectToDetach.transform.parent != transform ? attachedObjects.FindIndex(l => l.attachedObject == objectToDetach.transform.parent.gameObject) :
                attachedObjects.FindIndex(l => l.attachedObject == objectToDetach);
            ...
            if (index != -1)
            {
                ...
                if (hoverables.Length <= 0)
                    hoverables = attachedObjects[index].attachedObject.GetComponentsInChildren<Hoverable>();
                ...
                if (newTopObject != null && newTopObject != prevTopObject)
                {
                    ...
                    if (h.Length <= 0)
                        h = newTopObject.GetComponentsInChildren<Hoverable>();
        ...
        private void Awake()
        {
            ...
            if (hoverRayLocation == null)
                hoverRayLocation = transform;
        ...
        private void UpdateHovering()
        {
            ...
            RaycastHit[] hitColliders = Physics.SphereCastAll(hoverRayLocation.position, sphereCastRadius,
                hoverRayLocation.forward, maxThrowableDistance, hoverLayerMask);
        ...
        private void Update()
        {
            ...
            if (currentAttachedObject)
            {
                if (currentAttachedObject.GetComponentInChildren<Throwable>() != null)
                {
                    float step;
                ...
                if (hoverables.Length <= 0)
                    hoverables = currentAttachedObject.transform.GetComponentsInChildren<Hoverable>();
            ...
            if (hoveringInteractable)
            {
                ...
                if (hoverables.Length <= 0)
                    hoverables = hoveringInteractable.GetComponentsInChildren<Hoverable>();
