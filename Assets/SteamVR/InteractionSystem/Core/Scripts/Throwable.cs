...
namespace Valve.VR.InteractionSystem {
    ...
    public class Throwable : Interactable
    {
        ...
        public override void HandAttachedUpdate (Hand hand) {
            //For whatever reason make sure we are kinematic
            if (rBody.isKinematic == false)
                rBody.isKinematic = true;

            //Trigger got released
            if (!hand.GetStandardInteractionButton() && GetComponentInChildren<Renderer>() && GetComponentInChildren<Renderer>().enabled) {
